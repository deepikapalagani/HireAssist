...
# 2-Shot Examples (Used for both Llama and Mistral for consistency in this experiment, or as requested)
# User asked for 2-shot for Llama and 3-shot for Mistral previously, but now asks for "2-shot with two different sets of prompts".
# To keep it comparable across prompt styles, we'll use the same 2 examples for the "2-shot" experiments.
...
def parse_output(response):
    ...
    reasoning = ""
    try:
        # Regex to capture text between "Reasoning:" and "Decision:"
        # Case insensitive, dot matches newline
        match = re.search(r"reasoning:(.*?)(?:decision:|$)", response_lower, re.DOTALL | re.IGNORECASE)
        if match:
            reasoning = match.group(1).strip()
        else:
            # If explicit tag not found, take everything before "Decision:"
            parts = response.split("Decision:")
            if len(parts) > 1:
                reasoning = parts[0].strip()
            else:
                reasoning = response.strip() # Fallback: whole response is reasoning
    except Exception:
...
def main():
    parser = argparse.ArgumentParser(description="Run ICL Inference")
